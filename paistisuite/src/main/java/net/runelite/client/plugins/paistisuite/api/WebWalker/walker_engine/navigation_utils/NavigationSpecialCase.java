...
//        DIGSITE_BARGE(3724, 3808, 0),
    ...
    public static boolean handle(SpecialLocation specialLocation){
        switch (specialLocation){
            ...
            case WATERFALL_DUNGEON_ENTRANCE:
                ...
                {
                    if (WaitFor.condition(15000, () -> PPlayer.location().equals(new RSTile(2511, 3463, 0)) ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE) == WaitFor.Return.SUCCESS){
                        return true;
                    }
                }
            ...
            case ZEAH_SAND_CRAB:
                if (InteractionHelper.click(PObjects.findNPC(Filters.NPCs.nameEquals("Sandicrahb")), "Quick-travel") && WaitFor.condition(10000, () -> PPlayer.location().getY() >= 3457 ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE) == WaitFor.Return.SUCCESS){
    ...
    public static boolean clickObject(Predicate<PTileObject> filter, String[] action, WaitFor.Condition condition){
        List<PTileObject> objects = PObjects.getAllObjects()
                ...
                .sorted((a,b) -> Math.round(a.getFirst().getWorldLocation().distanceToHypotenuse(PPlayer.location())) - Math.round(b.getFirst().getWorldLocation().distanceToHypotenuse(PPlayer.location())))
